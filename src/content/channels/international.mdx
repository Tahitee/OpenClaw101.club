---
title: 接入 WhatsApp / Telegram / Discord
description: 接入国际主流即时通讯平台的完整教程
---
...
6. 进入 **OAuth2 → URL Generator**：
   - Scopes 选择 `bot`
   - Bot Permissions 至少选择：`Send Messages`、`Read Message History`、`View Channels`
...
2. **配置文件位置**：所有配置都在 `~/.openclaw/openclaw.json` 的 `channels` 字段中
3. **启动 Gateway**：配置完成后，运行 `openclaw gateway start` 或 `openclaw gateway restart` 使配置生效
