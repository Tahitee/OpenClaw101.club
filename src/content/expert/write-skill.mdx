...
description: "手把手教你创建、测试和发布一个 OpenClaw Skill，以天气查询为完整示例"
---
...
请将 JSON 数据转化为用户友好的格式，添加天气 emoji。
```
...
1. **查看 Agent 是否识别 Skill**：在对话中问「你有哪些 Skill？」
2. **检查脚本输出**：手动运行脚本确认输出格式正确
3. **查看错误日志**：`docker compose logs -f | grep "weather"`
4. **环境变量检查**：确保 `.env` 中设置了必需的 API Key

## 发布到 ClawdHub

[ClawdHub](https://hub.openclaw.com) 是 OpenClaw 的 Skill 商店。开发完成后，你可以将 Skill 发布供所有人使用。
...
# 🔗 https://hub.openclaw.com/skills/weather
```
...
1. **单一职责**：一个 Skill 专注一个领域（天气、邮件、日历等）
2. **优雅降级**：API Key 缺失时给出清晰的错误提示
3. **输出格式统一**：使用 JSON 输出，便于 Agent 解析
4. **文档完善**：SKILL.md 中的说明越清晰，Agent 使用越准确
5. **版本管理**：使用 git 管理 Skill 代码，语义化版本号
6. **最小依赖**：尽量使用标准库，减少安装步骤
